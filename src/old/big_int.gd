...
## Various options to control the string presentation of BigInt Numbers.
...
## Initializes the BigInt number.
## Can take another BigInt, a String (scientific notation), a float, or an int.
func _init(m: Variant = options["default_mantissa"], e: int = options["default_exponent"]) -> void:
	if m is BigInt:
		...
		if typeof(m) != TYPE_INT and typeof(m) != TYPE_FLOAT:
			printerr("BigInt Error: Unknown data type passed as a mantissa!")
			mantissa = 1.0
			exponent = 0
		else:
			mantissa = float(m)
...
## Converts the BigInt Number into a string.
...
## Normalizes the BigInt number (keeps mantissa between 1.0 and 10.0).
...
## Returns a new BigInt with the absolute value.
func absolute() -> BigInt:
	var result: BigInt = BigInt.new(self)
...
func is_less_than(n: Variant) -> bool:
	var other: BigInt = _type_check(n)
...
## Adds a number to this BigInt and returns a new BigInt.
func plus(n: Variant) -> BigInt:
	var res: BigInt = BigInt.new(self)
	res.plus_equals(n)
	return res


## adds a number to this BigInt in-place.
func plus_equals(n: Variant) -> BigInt:
	var other: BigInt = _type_check(n)
...
## Subtracts a number from this BigInt and returns a new BigInt.
func minus(n: Variant) -> BigInt:
	var res: BigInt = BigInt.new(self)
	res.minus_equals(n)
	return res


## Subtracts a number from this BigInt in-place.
func minus_equals(n: Variant) -> BigInt:
	var other: BigInt = _type_check(n)
...
## Multiplies this BigInt by a number and returns a new BigInt.
func multiply(n: Variant) -> BigInt:
	var res: BigInt = BigInt.new(self)
	res.multiply_equals(n)
	return res


## Multiplies this BigInt by a number in-place.
func multiply_equals(n: Variant) -> BigInt:
	var other: BigInt = _type_check(n)
...
## Divides this BigInt by a number and returns a new BigInt.
func divide(n: Variant) -> BigInt:
	var res: BigInt = BigInt.new(self)
	res.divide_equals(n)
	return res


## Divides this BigInt by a number in-place.
func divide_equals(n: Variant) -> BigInt:
	var other: BigInt = _type_check(n)
	
	if other.mantissa == 0.0:
		printerr("BigInt Error: Divide by zero")
		return self
...
## Modulo of this BigInt by a number.
func mod(n: Variant) -> BigInt:
	# Modulo: a - (n * floor(a/n))
	var other: BigInt = _type_check(n)
	
	var quot: BigInt = divide(other)
	quot.floor_value() # Round down
	
	var product: BigInt = quot.multiply(other)
	var res: BigInt = minus(product)
	return res


## Raises this BigInt to a power and returns a result.
func power(n: Variant) -> BigInt:
	var res: BigInt = BigInt.new(self)
	res.power_equals(n)
	return res


## Raises this BigInt to a power in-place.
func power_equals(n: Variant) -> BigInt:
	...
	if typeof(n) == TYPE_INT:
		...
		if p == 0:
			...
			return self
		if p < 0:
			printerr("BigInt Error: Negative integer exponents not fully supported in simple power!")
			return self
		...
		# Converting BigInt exponent to float for power calculation
...
## Converts the BigInt Number into a string (in Scientific format).
...
## Converts the BigInt Number into a string (in AA format).
...
## Converts the BigInt Number into a string (in Metric Symbols format).
...
## Converts the BigInt Number into a string (in Metric Name format).
...
func _size_check(m: float) -> void:
	if m > MANTISSA_MAX:
		printerr("BigInt Error: Mantissa \"" + str(m) + "\" exceeds MANTISSA_MAX.")


func _type_check(n: Variant) -> BigInt:
	if n is BigInt:
		return n
	else:
		return BigInt.new(n)
